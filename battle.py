...
# --- In-Memory Battle Management (unchanged) ---
ACTIVE_BATTLES = {}
BATTLE_INVITES = {}
...
def get_battle_text(players, turn_player_id):
    # (code is the same)
    player_ids = list(players.keys()); p1 = players[player_ids[0]] if player_ids[0] < player_ids[1] else players[player_ids[1]]; p2 = players[player_ids[1]] if player_ids[0] < player_ids[1] else players[player_ids[0]]
...
async def send_battle_menu(context: ContextTypes.DEFAULT_TYPE, battle_state, message_id=None):
    ...
    keyboard = [
        [InlineKeyboardButton("⚔️ Taijutsu", callback_data=f"battle_action_taijutsu_{turn_player_id}"), InlineKeyboardButton("🌀 Use Jutsu", callback_data=f"battle_action_jutsu_{turn_player_id}")],
        [InlineKeyboardButton("🧪 Use Item", callback_data=f"battle_action_item_{turn_player_id}"), InlineKeyboardButton("🏃 Flee", callback_data=f"battle_action_flee_{turn_player_id}")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    try:
        if message_id:
            await context.bot.edit_message_text(chat_id=battle_state['chat_id'], message_id=message_id, text=text, reply_markup=reply_markup, parse_mode="HTML")
            return message_id
        else:
            message = await context.bot.send_message(chat_id=battle_state['chat_id'], text=text, reply_markup=reply_markup, parse_mode="HTML")
...
async def battle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for player in [p1, p2]:
        if player['battle_cooldown']:
            ...
            if now < cooldown_time: remaining = (cooldown_time - now).seconds // 60 + 1; await update.message.reply_text(f"{player['username']} is on battle cooldown for {remaining} more minute(s)."); return
...
async def battle_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "jutsu":
        ...
        # --- THIS IS THE FIX ---
        if isinstance(attacker['known_jutsus'], str):
    ...
    if action == "item":
        # --- THIS IS THE FIX ---
        if isinstance(attacker['inventory'], str):
            inventory_list = json.loads(attacker['inventory'])
        else:
            inventory_list = attacker.get('inventory', [])
        # --- END OF FIX ---
...
async def battle_item_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    item_effect_text = ""
    # We must save inventory *and* any changed stats
    ...
    if item_key == 'health_potion':
        ...
        if actual_heal <= 0:
             ...
             await send_battle_menu(context, battle_state, message_id)
             return # Don't end turn, don't save (since nothing was used)
        else:
